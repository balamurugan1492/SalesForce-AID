...
namespace Pointel.Salesforce.Adapter
{
    ...
    public interface ISFDCListener
    {
        ...
        IDictionary<string, string> Click2EmailData
        {
            get;
        }

        IDictionary<string, string> ChatActivityIdCollection
        {
            get;
        }

        IDictionary<string, string> EmailActivityIdCollection
        {
            get;
        }

        EventGetInteractionContent GetOpenMediaInteractionContent(string InteractionId, bool includeAttachments);

        void MakeOutboundCall(string phoneNumber, string attachDataKey, string attachDataValue);

        void MakeConsultCall(string phoneNumber, string attachDataKey, string attachDataValue, ConnectionId connId);

        void MakeOutboundEmail(string emailAddress, string recordId, string objectName);

        void SetAttachedData(string interactionId, string key, string value, int proxyClienntID);

        string GetVoiceComments(string connId);

        IMessage GetEmailAttachment(string documentID, bool canOpen);
    }
}