    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (AdapterData && AdapterData.Event == "init") {
                enableEditPrompt = AdapterData.EnablePrompt;
            }
...
function init() {
    ...
    try {
        ...
        setTimeout('sendSessionToAdapter()', 500);
        setTimeout('sendTimezoneToAdapter()', 500);
        $.ajax({
            url: Url + "/opened",
            dataType: "jsonp",
            crossDomain: true,
...
function sendSessionToAdapter() {
    ...
    /// <returns></returns>
    try {
        sforce.interaction.runApex('SessionId', 'GetSessionId', '', function (response) {
            if (response.result) {
                $.ajax({
