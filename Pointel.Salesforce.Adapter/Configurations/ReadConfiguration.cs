...
namespace Pointel.Salesforce.Adapter.Configurations
{
    ...
    internal class ReadConfiguration
    {
        ...
        public KeyValueCollection ReadSFDCUtilityConfig(CfgApplication myApplication, List<CfgAgentGroup> agentGroups, CfgPerson personObject, string sectionName)
        {
            try
            {
                logger.Info("ReadSFDCUtilityConfig : Reading SFDC Objects Configuration");
                logger.Info("ReadSFDCUtilityConfig : Object Name : " + sectionName);
                KeyValueCollection SFDCConfig = null;
                ...
                try
                {
                    if (myApplication != null)
                    {
                        ...
                        if (myApplication.UserProperties.ContainsKey(sectionName))
                        {
                            ...
                            this.logger.Info("ReadSFDCUtilityConfig : " + (SFDCConfig != null ? "Read " + sectionName + " Configuration at Application Level" : " No configuration found at Application Level"));
                        }
                        else
                        {
                            this.logger.Info("ReadSFDCUtilityConfig : " + sectionName + " Configuration is not found at Application Level.");
                        }
                    }
                }
                catch (Exception generalException)
                {
                    this.logger.Error("ReadSFDCUtilityConfig : Error occurred while reading configuration at application level :" + generalException.ToString());
                }
                        ...
                        foreach (CfgAgentGroup agentgroup in agentGroups)
                        {
                            if (agentgroup.GroupInfo.UserProperties.ContainsKey(sectionName))
                            {
                                ...
                                if (agentGroupConfig != null)
                                {
                                    ...
                                    logger.Info("ReadSFDCUtilityConfig : " + sectionName + " configuration taken from Agent Group : " + agentgroup.GroupInfo.Name);
                                    break;
                                }
                            }
                            else
                            {
                                logger.Info("ReadSFDCUtilityConfig : " + sectionName + " configuration is not found at Agent Group : " + agentgroup.GroupInfo.Name);
                            }
                        }
                    }
                }
                ...
                {
                    this.logger.Error("ReadSFDCUtilityConfig : Error occurred while reading configuration at AgentGroup level :" + generalException.ToString());
                }
                ...
                try
                {
                    ...
                    {
                        logger.Info("ReadSFDCUtilityConfig : " + sectionName + " configuration is not found at Agent Level");
                    }
                }
                catch (Exception generalException)
                {
                    this.logger.Error("ReadSFDCUtilityConfig : Error occurred while reading configuration at Agent level :" + generalException.ToString());
                }

                #endregion Reading General Configurations

                if (SFDCConfig != null)
                    logger.Info("ReadSFDCUtilityConfig : " + sectionName + " Configuration Data : " + SFDCConfig.ToString());

                return SFDCConfig;
            }
            catch (Exception generalException)
            {
                logger.Error("ReadSFDCUtilityConfig : Error Occurred while reading SFDC Configuration : " + generalException.ToString());
            }
            return null;
        }
