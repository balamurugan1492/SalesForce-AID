namespace Pointel.TaskbarNotifier
{
    using System;
    ...
    public class TaskbarNotifier : System.Windows.Window, INotifyPropertyChanged
    {
        ...
        public bool isAutoAnswerTimer = false;
        public INotifier objiNotifier;
        ...
        public void Notify(double windowHeight, bool isbottom)
        {
            try
            {
                Isbottom = isbottom;
        ...
        private void stayOpenTimer_Elapsed(Object sender, EventArgs args)
        {
            ...
            this.stayOpenTimer.Stop();
            if (objiNotifier != null)
                objiNotifier.Notify();
            if (isAutoAnswerTimer || !this.IsMouseOver)
            {
                // Only start closing the window if the mouse is not over it.
                this.DisplayState = DisplayStates.Hiding;

                //if (System.Windows.Application.Current.Windows.Cast<Window>().Where(x => x.Title == "EmailNotifier").ToList().Count > 0)
                //{
                //    INotifier objTaskbarNotifier = (INotifier)System.Windows.Application.Current.Windows.Cast<Window>().Where(x => x.Title == "EmailNotifier").SingleOrDefault();
                //    if (objTaskbarNotifier != null)
                //        objTaskbarNotifier.Notify();
                //}
